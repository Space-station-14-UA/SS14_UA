...
public abstract partial class SharedTetherGunSystem : EntitySystem
{
    ...
    private void OnTetherMove(RequestTetherMoveEvent msg, EntitySessionEventArgs args)
    {
        ...
        EntityUid? gunUid;
        TetherGunComponent? gun = null;
        TetherForseCombineComponent? fgun = null;

        if ((!TryGetTetherGun(user.Value, out gunUid, out gun) || gun.TetherEntity == null) && (!TryGetTetherForceGun(user.Value, out gunUid, out fgun) || fgun.TetherEntity == null))
        {
            return;
        }
        ...
        if (fgun != null)
        {
            if (!coords.TryDistance(EntityManager, TransformSystem, Transform(gunUid.Value).Coordinates,
                out var distance) ||
            distance > fgun.MaxDistance)
            {
                return;
            }
        }

        if (gun != null && gun.TetherEntity != null)
            TransformSystem.SetCoordinates(gun.TetherEntity.Value, coords);
        else if (fgun != null && fgun.TetherEntity != null)
            TransformSystem.SetCoordinates(fgun.TetherEntity.Value, coords);
    }
    ...
    protected bool TryGetTetherForceGun(EntityUid user, [NotNullWhen(true)] out EntityUid? gunUid, [NotNullWhen(true)] out TetherForseCombineComponent? gun)
    {
        gunUid = null;
        gun = null;
