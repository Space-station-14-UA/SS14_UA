...
public sealed partial class TetherGunSystem : SharedTetherGunSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<TetherForseCombineComponent, AfterAutoHandleStateEvent>(OnAfterState);
    ...
    public override void Update(float frameTime)
    {
        ...
        if (player == null ||
            (!TryGetTetherGun(player.Value, out _, out var gun) || gun.TetherEntity == null)
            && (!TryGetTetherForceGun(player.Value, out _, out var fgun) || fgun.TetherEntity == null)
            )
        {
            return;
        }
        ...
        if (TryGetTetherGun(player.Value, out _, out var tgun))
        {
            if (TryComp(tgun.TetherEntity, out TransformComponent? tetherXform) &&
        ...
        else if (TryGetTetherForceGun(player.Value, out _, out var tfgun))
        {
            if (TryComp(tfgun.TetherEntity, out TransformComponent? tetherXform) &&
            tetherXform.Coordinates.TryDistance(EntityManager, TransformSystem, coords, out var distance) &&
            distance < bufferDistance)
            {
                return;
